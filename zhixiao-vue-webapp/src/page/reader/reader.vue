<template>
  <div
    class="page-reader" id="page-reader"
    :style="{color: bookMessage.color, backgroundColor: bookMessage.backgroundColor}"
    ref='viewBox'
    @click="isShow = true"
    v-if="isOk"
  >
    <div class="reader-content-details" v-show='isLoading'>
      <div
        :ref="`readerChapter${item.chapter}`"
        :id="`readerChapter${item.chapter}`"
        v-for="item in textContent"
        :key="item.chapter"
      >
      ...
      <div v-show="isBookLoadingShow" class="scroll-loading">
        <div class="loader">
    ...
    <div class="page-read-option" :class="{showPanel: isShow}" v-on:click.stop="panelClick($event)">
      ...
      <div class="bottom-controll-panel" ref="panelBottom">
        ...
        <div class="item-footer">
          <span class="chapter-btn-w btn" @click="previousChapter()">上一章</span>
          <span class="directory btn" @click="directoryShowClick()">目录</span>
          <span class="chapter-btn-w btn f-r" @click="nextChapter()">下一章</span>
        </div>
      </div>
    </div>
    <!-- 目录 -->
    <div class="page-read-chapter-list" :class="{'show-panel': directoryShow}" @click.stop="directoryClick($event)">
      <div class="content" ref="directory">
        <div class="top-bar" ref="directory_title">
          <span>{{ bookTitle }}</span>
          <span>目录</span>
        </div>
        <ul class="directory-list" ref="directory_list" :style="directoryStyle">
          <li v-for="(item, index) in bookChapters" :key="index">
            <a @click="readerDirectory(index + 1)">
              ...
              <span v-if="item.isVip" class="vip"></span>
            </a>
          </li>
        </ul>
      </div>
...
export default {
  ...
  mounted() {
    ...
    this.$nextTick(() => {
      // console.log('directory_title', document.body.clientHeight, this.$refs)
      //  获取单个目录高度
      // console.log('this.directoryListHeight', this.$refs.directory_list.childNodes[0].getBoundingClientRect())
      this.directoryListHeight = this.$refs.directory_list.childNodes[0].getBoundingClientRect().height
      this.directoryStyle.height = document.body.clientHeight - this.$refs.directory_title.offsetHeight + 'px'
    })
  },
  ...
  methods: {
    ...
    getBookSources: function (book_id, isClickDirectory) {
      this.axios.get(API.book.bookSources(book_id))
        .then((res) => {
          ...
          this.getBookChapters(res.data[0]._id, isClickDirectory)
        })
        .catch((res) => {
    ...
    getBookChapters: function (book_id, isClickDirectory) {
      this.axios.get(API.book.bookChapters(book_id))
        .then((res) => {
          ...
          this.chapterContent(bookIndex.link, bookIndex.title, this.$route.query.index, isClickDirectory)
        })
        .catch((res) => {
          this.isOk = false;
          this.isLoading = true;
    ...
    // 获取章节内容
    chapterContent: function (link, title, chapter, isClickDirectory) {
      let _this = this
      this.axios.get(API.book.chapterContent(link))
        .then((res) => {
          // console.log('获取章节内容-成功', res)
          // 获取到的章节内容没有p标签，使其每段分割，组成数组
          // let bodyArray = res.data.chapter.cpContent.split(/^\s*|\s*$/g,"")
          if(res.data.chapter.isVip) {
             res.data.chapter.cpContent = "付费章节，请移至正版网站阅读"
          }
          let bodyArray = res.data.chapter.cpContent.split('\r\n')
          bodyArray = res.data.chapter.cpContent.split('\n')
          // 给每段加上p标签
          ...
          bookContentArr.chapter = chapter
          isClickDirectory ? (this.textContent = []) : ''
          this.$refs.directory_list.scrollTop = this.directoryListHeight * (this.$route.query.index - 1)
          this.textContent.push(bookContentArr)
          this.isLoadingChapter = true
          this.isLoading = true
          this.isBookLoadingShow = false
          // console.log("directory scrollTop", this.$refs)
          if(this.jiazai) {
            this.jiazai = false;
            this.$nextTick(() => {
              this.handleScroll()
    ...
    handleScroll: function () {
      const _this = this;
      // 获取节点
      var scrollDiv = document.getElementById('page-reader');
      // 滚动事件
      scrollDiv.addEventListener('scroll', function() {
        var windowHeight = document.body.clientHeight;
        if((scrollDiv.scrollHeight - windowHeight / 5) < scrollDiv.clientHeight + scrollDiv.scrollTop && scrollDiv.clientHeight + scrollDiv.scrollTop < scrollDiv.scrollHeight) {
          _this.isBookLoadingShow = true
          // div 到头部的距离 + 屏幕高度 = 可滚动的总高度
          if(_this.isLoadingChapter) {    // 防止多次执行
            _this.isLoadingChapter = false;
            _this.chapterLoading();
            _this.getBookSources(_this.$route.query.book_id);
            for (let i in _this.bookIdMessage) {
              if(_this.bookIdMessage[i].book_id === _this.$route.query.book_id) {
                _this.bookIdMessage[i].index = _this.$route.query.index;
              }
            }
            // console.log('_this.bookIdMessage', _this.bookIdMessage)
            store.set('bookIdMessage', _this.bookIdMessage);
          }
        }
      }, false);
    },
    ...
    chapterLoading: function (index) {
      ...
      var bookcase = store.get('bookcase');
      if(bookcase) {
        for(var i = 0; i < bookcase.length; i ++) {
          if(bookcase[i].book_id === this.$route.query.book_id) {
            bookcase[i].index = newQuery.index;
          }
        }
      }
      store.set('bookcase', bookcase);
    },
    ...
    previousChapter: function () {
      this.isLoading = false;
      this.chapterLoading(Number(this.$route.query.index) - 1);
      this.getBookSources(this.$route.query.book_id, true);
    },
    // 下一章
        ...
        .directory-list{
          ...
          li{
            ...
            .vip{
              ...
              width: .7467rem;
              height: .8533rem;
              background: url(data:image/png;base64,) 50% no-repeat;
